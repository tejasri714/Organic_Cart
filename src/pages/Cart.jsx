...
const Cart = () => {
  ...
  const placeOrder = async () => {
    alert(`Order placed with ${paymentOption}`);
  };
  ...
  return products.length > 0 && cartItems ? (
    <div className="flex flex-col md:flex-row mt-16 gap-8 px-6">
      {/* Left - Cart Items */}
      <div className="flex-1 max-w-4xl">
        ...
        {cartArray.map((product, index) => (
          <div
            key={index}
            className="grid grid-cols-[2fr_1fr_1fr] items-center text-sm md:text-base font-medium pt-3 border-b border-gray-200 py-4"
          >
      ...
      {/* Right - Order Summary */}
      <div className="max-w-[360px] w-full bg-gray-100/40 p-5 border border-gray-300/70 rounded-md">
        <h2 className="text-xl font-medium text-primary">Order Summary</h2>
        <hr className="border-gray-300 my-5" />

        <div className="mb-6">
          <p className="text-sm font-medium uppercase">Delivery Address</p>
          <div className="relative flex justify-between items-start mt-2">
            <p className="text-gray-500 text-sm w-[200px]">
              {selectedAddress
            ...
            {showAddress && (
              <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full z-10 shadow-md">
        ...
        <button
          onClick={placeOrder}
          className="w-full py-3 mt-6 cursor-pointer btn-primary font-medium rounded"
        >
